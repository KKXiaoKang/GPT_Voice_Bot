...
def run_gpt_prompt_insight_and_guidance(statements, n, extra_prompt:str=None, verbose=False):

  def create_prompt_input(statements, n, extra_input=None): 
    prompt_input = [statements, str(n), extra_input]
    return prompt_input
  ...
  def __func_clean_up(gpt_response, prompt=""):
    ...
    for i in gpt_response.split("\n"): 
      row = i.split(". ")[-1]
      thought = row.split("(因为")[0].strip()
      evi_raw = row.split("(因为")[1].split(")")[0].strip()
  ...
  gpt_param = {"engine": "text-davinci-003", "max_tokens": 150, 
               "temperature": 0.5, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = os.path.join(FILE_PATH, "prompt_template/v2/insight_and_evidence_v1.txt")
  prompt_input = create_prompt_input(statements, n, extra_input=extra_prompt)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_map_node(event_description, test_input=None, verbose=False): 
  def create_prompt_input(event_description, test_input=None): 
    prompt_input = [event_description]
    return prompt_input
  ...
  def __chat_func_validate(gpt_response, prompt=""): ############   
    try: 
      ...
      for curr_gpt_response in gpt_response:
        if "time" in curr_gpt_response and "subject" in curr_gpt_response and "location" in curr_gpt_response and "state" in curr_gpt_response:
          return True
        else:
          return False
  ...
  gpt_param = {"engine": "text-davinci-002", "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = os.path.join(FILE_PATH, "prompt_template/v3_ChatGPT/map_node_from_descript_v1.txt" )########
  prompt_input = create_prompt_input(event_description)  ########
  prompt = generate_prompt(prompt_input, prompt_template)
  example_output = "5" ########
  special_instruction = "Return json, which contains: object, location, time, status." ########
  fail_safe = get_fail_safe() ########
  output = ChatGPT_map_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
...
def run_gpt_prompt_focal_pt(statements, n, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "text-davinci-003", "max_tokens": 15,
              "temperature": 0, "top_p": 1, "stream": False,
              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = os.path.join(FILE_PATH, "prompt_template/v3_ChatGPT/generate_focal_pt_v1.txt") ########
  prompt_input = create_prompt_input(statements, n)  ########
  prompt = generate_prompt(prompt_input, prompt_template)
  example_output = '["需要做什么来帮助主人", "房间里面有什么", "是否需要调暗灯光"]' ########
  special_instruction = "Output must be a list of str." ########
  fail_safe = get_fail_safe(n) ########
  
  output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
                                          __chat_func_validate, __chat_func_clean_up, False) # 小心问题
  
  print("focal point output: ", output)
